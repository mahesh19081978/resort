...
describe("C. CANONICAL PRICING ENGINE — calculateBookingPrice() with mocked DB", () => {
  function makeClient(opts: { taxRate?: string; taxFound?: boolean; roomFound?: boolean } = {}) {
    ...
    return {
      ...
      tax: {
        findMany: vi.fn().mockResolvedValue(
          taxFound
            ? [{ id: "t1", code: "ROOM_GST", rate: new Prisma.Decimal(taxRate), isActive: true, name: "Room GST", scope: TaxScope.ROOM, effectiveFrom: null, effectiveTo: null }]
            : []
        ),
      },
  ...
  it("C2 — CRITICAL: Fails closed when no ROOM tax is configured — no silent fallback", async () => {
  ...
  it("C7 — Tax lookup uses scope=ROOM and isActive=true (scope-based resolver, not hardcoded code)", async () => {
