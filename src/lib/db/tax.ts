...
export const TAX_CODES = {
  ROOM_GST: 'ROOM_GST',
  SERVICE_DEFAULT: 'GST_SVC_18',
} as const;

/**
 * Lookup an active tax rate by its unique code from the authoritative Tax table.
 * Returns the resolved tax info (code, rate, name).
 * Throws if the tax code does not exist, is inactive, or is not within effective dates.
 */
export async function getTaxRateByCode(
  ...
  if (!taxRecord) {
    throw new Error(
      `TAX_CONFIG_MISSING: Active, effective tax configuration with code '${code}' not found in database.`
    );
  }
...
/**
 * Deterministically resolves the authoritative Tax for Room accommodation at a given transaction time.
 *
 * Scope-based resolution (no hardcoded tax codes):
 *   Queries Tax records with scope = TaxScope.ROOM, isActive = true, and within effective date window.
 *   - count === 0 -> throws TAX_CONFIG_MISSING
 *   - count > 1   -> throws TAX_CONFIG_AMBIGUOUS (no arbitrary order by / pick)
 *   - count === 1 -> returns resolved tax
 *
 * The asOf parameter MUST be a single authoritative timestamp passed through the entire
 * pricing calculation to prevent inconsistent boundary behavior.
 */
export async function resolveTaxForRoom(
  atTime: Date,
  client: TaxClient = prisma
): Promise<ResolvedTax> {
  const matchingTaxes = await client.tax.findMany({
  ...
  if (matchingTaxes.length === 0) {
    throw new Error(
      `TAX_CONFIG_MISSING: No active, effective tax found for scope 'ROOM' at ${atTime.toISOString()}. Configure a ROOM-scope tax in Settings > Taxes.`
    );
  }
  ...
  if (matchingTaxes.length > 1) {
    const codes = matchingTaxes.map((t) => t.code).join(', ');
    throw new Error(
      `TAX_CONFIG_AMBIGUOUS: Multiple active taxes found for scope 'ROOM' at ${atTime.toISOString()}: [${codes}]. Resolve tax ambiguity in Settings > Taxes.`
    );
  }
