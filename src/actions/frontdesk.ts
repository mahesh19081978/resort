...
// Server-authoritative tax: client must NOT submit taxAmount.
// Client submits taxCode (optional) + idempotencyKey.
// Server computes tax from canonical Tax table.
// ----------------------------------------------------
export async function postFolioChargeAction(
  ...
  try {
    ...
    const raw = {
      ...
      taxCode: formData.get('taxCode')?.toString() || undefined,
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        if (!existing.unitPrice.equals(baseAmount) || existing.description !== parsed.data.description) {
          throw new Error(
            `IDEMPOTENCY_KEY_REUSE_CONFLICT: Idempotency key '${parsed.data.idempotencyKey}' was previously used for a materially different charge.`
          );
        }
        return existing;
      }

      // 2. Server-authoritative tax computation via canonical tax resolution
      const baseAmount = new Prisma.Decimal(parsed.data.amount);
      let taxAmount = new Prisma.Decimal(0);

      if (parsed.data.taxCode) {
        // Use canonical tax resolution: validates isActive, effectiveFrom, effectiveTo
        const taxRate = await getTaxRateByCode(parsed.data.taxCode, tx);
      ...
      let item;
      try {
        item = await tx.folioItem.create({
          data: {
            folioId: folio.id,
        ...
        if (err && typeof err === 'object' && 'code' in err && (err as { code: string }).code === 'P2002') {
          ...
          if (raceExisting) {
            const baseAmount2 = new Prisma.Decimal(parsed.data.amount);
            if (!raceExisting.unitPrice.equals(baseAmount2) || raceExisting.description !== parsed.data.description) {
              throw new Error(
                `IDEMPOTENCY_KEY_REUSE_CONFLICT: Idempotency key '${parsed.data.idempotencyKey}' was previously used for a materially different charge.`
              );
            }
      ...
      // corruption under concurrent transactions.
